...
void ResetDeckardXParams()
{
    ...
    */
    ...
    1 DECKARD IOP
    */
   DPRINTF("Resetting XPARAM parameters\n");
    if ((*GM_IF & GM_IOP_TYPE) == 0)
    {
        DPRINTF("CONSOLE IS NOT DECKARD.\n");
        return;
    }
    ...
    if (fd >= 0) {
        DPRINTF("PS3/PS4 EMU DETECTED! Skipping reset\n");
        close(fd);
        return;
    }
...
void ApplyDeckardXParam(const char *title)
{
    ...
    if (title == NULL)
    {
        DPRINTF("TITLE IS NULL, ABORTING\n");
        return;
    }
    ...
    */
    ...
    if (fd >= 0) {
        close(fd);
        DPRINTF("PS3/PS4 EMU DETECTED! CALLING 'rom0:XPARAM2 INSTEAD\n");
